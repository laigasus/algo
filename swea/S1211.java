...
class S1211 {
    ...
    private static void dfs(int start, int y, int x, int ladderIdx, int depth) {
        if (y == SIZE - 1) {
            if (depth <= min) {
                result = start;
                min = depth;
        ...
        for (int i = 0; i < dx.length; i++) {
            ...
            if (isInside(y, nx) && data[y][nx] == 1) {
                ...
                int length = Math.abs(nx - x) + 1;
                dfs(start, y + 1, nx, ladderIdx, depth + 1 + length);
                return;
            }
        }

        dfs(start, y + 1, x, ladderIdx, depth + 1);
    }
